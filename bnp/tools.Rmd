The following sub-sections briefly describe the toolset required to run the example application code and provide instructions for their installation and usage. 
...
__crosscat__, __cgpm__, and __trcrpm__ are all implemented in __python 2.7__ and tested on __ubuntu 16.04__.  The following instruction use __docker__ to build an image with the required software bundled.  __docker__ is then used to deploy the image, as a container, from which the example application code is run.  To succesfully complete the following instructions you have root access to a local system running __ubuntu 18.04__ and with a working __git__ installation.
